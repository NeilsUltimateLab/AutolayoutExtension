...
/// A Less than or equal to relation between the two views.
///
/// This excepts the argments as a `KeyPath` for parent and child `UIView`.
///
/// This is equivalent to `.constraint(lessThanOrEqualTo:)` method defined on `NSLayoutAnchor`.
///
/// - Usage: - 
/// ```swift
/// self.view.anchor(anotherView) {
///     LessThanOrEqualTo(\.leadingAnchor)
///     LessThanOrEqualTo(\.trailingAnchor, titleLabel.leadingAnchor)
/// }
/// ```
public struct LessThanOrEqualTo<L, Axis>: ConstraintProvider where L: NSLayoutAnchor<Axis> {
    ...
    ///
    /// This is equivalent to this code, where both anchor for parent and child are same.
    /// ```swift
    /// self.label.leadingAnchor.constraint(lessThanOrEqualTo: view.leadingAnchor)
    /// ```
    ...
    ///     LessThanOrEqualTo(\.centerYAnchor, titleLabel.centerYAnchor)
    ///   }
    ///   ```
    ...
    public func constraint(_ parent: UIView, _ child: UIView) -> some ConstraintType {
        let constraint = child[keyPath: from].constraint(lessThanOrEqualTo: siblingLayoutAnchor ?? parent[keyPath: toKeyPath])
        return AnyConstraint(constraint)
    }
}

/// An ``LessThanOrEqualTo`` varient where the `KeyPath` leads to properties of `NSLayoutDimension` class.
///
/// This is equivalent to `.constraint(lessThanOrEqualToConstant:)` declared on `NSLayoutDimension`.
public struct LessThanOrEqualToConstant<L>: ConstraintProvider where L: NSLayoutDimension {
    ...
    ///   ```swift
    ///   self.view.anchor(dateLabel) {
    ///     LessThanOrEqualToConstant(\.widthAnchor, constant: 120)
    ///   }
    ///   ```
    ...
    public func constraint(_: UIView, _ child: UIView) -> some ConstraintType {
        let constraint = child[keyPath: keyPath].constraint(lessThanOrEqualToConstant: constant)
        return AnyConstraint(constraint)
    }
}
#endif
